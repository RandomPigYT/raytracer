...
/*
 ...
 Functions:
   ...
   CGLM_INLINE versors glms_quat_forp(vec3s from, vec3s to, vec3s fwd, vec3s
 up) CGLM_INLINE vec3s   glms_quat_rotatev(versors q, vec3s v) CGLM_INLINE
 mat4s   glms_quat_rotate(mat4s m, versors q) CGLM_INLINE mat4s
 glms_quat_rotate_at(mat4s m, versors q, vec3s pivot) CGLM_INLINE mat4s
 ...
 */
CGLM_INLINE
versors glms_quat_from_vecs(vec3s a, vec3s b) {
  versors dest;
 ...
 */
CGLM_INLINE
float glms_quat_norm(versors q) { return glm_quat_norm(q.raw); }
 ...
 */
CGLM_INLINE
float glms_quat_dot(versors p, versors q) { return glm_quat_dot(p.raw, q.raw); }
 ...
 */
CGLM_INLINE
float glms_quat_imaglen(versors q) { return glm_quat_imaglen(q.raw); }
 ...
 */
CGLM_INLINE
float glms_quat_angle(versors q) { return glm_quat_angle(q.raw); }
 ...
 */
CGLM_INLINE
versors glms_quat_nlerp(versors from, versors to, float t) {
  versors dest;
