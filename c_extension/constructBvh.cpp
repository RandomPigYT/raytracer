...
int64_t findParentSibling(Node nodes, int64_t* parents, uint32_t node,
                          int64_t parent) {
...
void parseBvh(struct bvh_t* bvh,
              FastBVH::BVH<float, struct triangle_t>& rawBvh) {
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    if (isInternalLeft(nodes, i, parents[i])) {
      bvh[i].missIndex =
          parents[i] + nodes[parents[i]].right_offset;  // Sibling
      continue;
    }
