...
vec4* getCorners(struct vertex_t* verts, uint32_t* triangles) {
  ...
  for (int i = 0; i < vector_size(triangles); i++) {
		...
		// Finds the smallest coordinates for each axis from the vertices of a triangle, and then sets it as the 'minCorner' if it is lower than 'minCorner'
		for (int j = 0; j < 3; j++){
			float temp = MIN(verts[3 * triangles[i]].position[j], verts[3 * triangles[i] + 1].position[j]);
			temp = MIN(temp, verts[3 * triangles[i] + 2].position[j]);

			minCorner[j] = MIN(minCorner[j], temp);
		}

		// Finds the largest coordinates for each axis from the vertices of a triangle, and then sets it as the 'minCorner' if it is greater than 'minCorner'
		for (int j = 0; j < 3; j++){
			float temp = MAX(verts[3 * triangles[i]].position[j], verts[3 * triangles[i] + 1].position[j]);
...
vec4* constructVolumes(struct sceneInfo_t* s, vec4* centroids,
                       ...
                       float bucketWidth, enum axis_e axis, uint32_t* numLeft, uint32_t* numRight) {
...
vec4* optimalVolumeInAxis(float* cost, enum axis_e axis, struct sceneInfo_t* s,
  ...
  for (int32_t i = 0; i < NUM_BUCKETS; i++) {
		...
		vec4* v = constructVolumes(s, centroids, parentNode, parentInfo, i, bucketWidth, axis, &numLeftTris, &numRightTris);
		...
		parentSurfaceArea = parentSurfaceArea > FLT_EPSILON ? parentSurfaceArea : FLT_EPSILON; // To avoid division by 0

		float p_a = leftSurfaceArea / parentSurfaceArea;
		float p_b = rightSurfaceArea / parentSurfaceArea;

		float tempCost = 0.125f + (numLeftTris * p_a) + (numRightTris * p_b); // C = t_trav + p_a * n * t_isect + p_b * n * t_isect
