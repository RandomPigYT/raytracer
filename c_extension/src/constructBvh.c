...
vec4* constructVolumes(struct sceneInfo_t* s, vec4* centroids, struct bvh_t* parentNode, 
	...
	for (int i = 0; i < vector_size(parentInfo->triangles); i++){
		...
		if (centroids[parentInfo->triangles[i]][axis] <= parentNode->corner1[axis] + ((bucketIndex + 1) * bucketWidth))
...
vec4* optimalVolumeInAxis(float* cost, enum axis_e axis, struct sceneInfo_t* s, vec4* centroids, 
													struct bvh_t* parentNode, struct bvhNodeInfo_t* parentInfo){
	float bucketWidth = fabs(parentNode->corner1[axis] - parentNode->corner2[axis]) / NUM_BUCKETS; 
...
// Returns a pointer to an array which contains four coordinates, 0 and 1 being the ones that describe the first volume,
// and 2 and 3 being the ones that describe the second volume.
vec4* findOptimalVolumes(struct sceneInfo_t* s, vec4* centroids, struct bvh_t* parentNode, 
...
void constructTree(struct bvh_t** b, struct bvhNodeInfo_t** bvhInfo, struct sceneInfo_t* s, 
									 vec4* centroids, int64_t parent) {
