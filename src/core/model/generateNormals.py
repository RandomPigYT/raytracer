...
def generateNormals(self):
    for i in range(0, len(self.vertices), 3):
        ...
        edge1 = glm.vec3(self.vertices[i + 1].position) - glm.vec3(self.vertices[i].position)
        edge2 = glm.vec3(self.vertices[i + 2].position) - glm.vec3(self.vertices[i].position)
